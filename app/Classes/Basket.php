...
class Basket
{
    ...
    public function __construct($createOrder = false)
    {
        ...
        if (is_null($order) && $createOrder) {
            ...
            $data['currency_id'] = CurrencyConversation::getCurrentCurrencyFromSession()->id;
    ...
    public function countAvailable($updateCount = false)
    {
        ...
        foreach ($this->order->products as $orderProduct) {
            ...
            if ($orderProduct->countInorder > $orderProduct->count) return false;
            if ($updateCount) {
                $orderProduct->count -= $orderProduct->countInOrder;
                $products->push($product);
            }
        }
        if ($updateCount) $products->map->save();
    ...
    public function addProduct(Product $product)
    {
        if ($this->order->products->contains($product)) {
            ...
            if ($pivotRow->countInOrderw >= $product->count) return false;
